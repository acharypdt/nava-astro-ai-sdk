import { AstroChartData, CalculationParams, SadeSatiResult, ShadbalaResult, AshtakavargaResult, VarshaphalResult } from './types';
import { Body, GeoVector, Ecliptic, Observer, SiderealTime } from 'astronomy-engine';
...
function jdFromDate(year: number, month: number, day: number, hour: number, minute: number, timezone: number): number {
  return 367 * year - Math.floor(7 * (year + Math.floor((month + 9) / 12)) / 4) + Math.floor(275 * month / 9) + day + 1721013.5 + ((hour + minute / 60 - timezone) / 24);
}

function getPlanetLongitude(body: Body, jd: number, ayanamsaDeg: number): number {
  const vector = GeoVector(body, jd, true);
  const eq = Ecliptic(vector);
  const geoLong = Math.atan2(eq.vec.y, eq.vec.x) * (180 / Math.PI);
  return ((geoLong - ayanamsaDeg) % 360 + 360) % 360;
}

function getPlanetSign(body: Body, jd: number, ayanamsaDeg: number): number {
  return Math.floor(getPlanetLongitude(body, jd, ayanamsaDeg) / 30) + 1;
}
...
export async function calculateChart(params: CalculationParams): Promise<AstroChartData> {
  ...
  const jdUt = jdFromDate(year, month, day, hour, minute, timezone);
  ...
  for (const [name, body] of Object.entries(PLANET_BODIES)) {
    try {
      const correctedLong = getPlanetLongitude(body, jdUt, ayanamsaDeg);
      const sign = Math.floor(correctedLong / 30) + 1;
      const house = 1;
      planets[name] = { name, longitude: correctedLong, sign, house, is_retrograde: false };
    } catch { continue; }
  }

  const ascLongitude = ((lstDeg - ayanamsaDeg) % 360 + 360) % 360;
  ...
  planets['Rahu'] = { name: 'Rahu', longitude: rahuLong, sign: rahuSign, house: ((rahuSign - ascSign + 12) % 12) + 1, is_retrograde: false };

  const ketuLong = ((180 - ayanamsaDeg) % 360 + 360) % 360;
  const ketuSign = Math.floor(ketuLong / 30) + 1;
  planets['Ketu'] = { name: 'Ketu', longitude: ketuLong, sign: ketuSign, house: ((ketuSign - ascSign + 12) % 12) + 1, is_retrograde: false };
  ...
  for (const [name, p] of Object.entries(planets)) {
    ...
    d9Planets[name] = { name, sign: d9Sign, house: d9House, longitude: d9Long, is_retrograde: false };
  }
  ...
  const now = new Date();
  const nowJd = jdFromDate(now.getFullYear(), now.getMonth() + 1, now.getDate(), now.getHours(), now.getMinutes(), 0);

  const simplifiedTransits: AstroChartData['transits'] = {};
  for (const [name, body] of Object.entries(PLANET_BODIES)) {
    try {
      const correctedLong = getPlanetLongitude(body, nowJd, ayanamsaDeg);
      simplifiedTransits[name] = { name, sign: Math.floor(correctedLong / 30) + 1 };
    } catch { continue; }
  }

  const d9LagnaSign = ascSign;
...
function getSaturnSignAtDate(date: Date): number {
  const jd = jdFromDate(date.getUTCFullYear(), date.getUTCMonth() + 1, date.getUTCDate(), date.getUTCHours(), date.getUTCMinutes(), 0);
  const ayanamsaDeg = calculateAyanamsaDeg(jd);
  const correctedLong = getPlanetLongitude(Body.Saturn, jd, ayanamsaDeg);
...
function getSunSiderealPosition(date: Date): number {
  const jd = jdFromDate(date.getUTCFullYear(), date.getUTCMonth() + 1, date.getUTCDate(), date.getUTCHours(), date.getUTCMinutes(), 0);
  const ayanamsaDeg = calculateAyanamsaDeg(jd);
  return getPlanetLongitude(Body.Sun, jd, ayanamsaDeg);
}

export function calculateSadeSati(moonSign: number): SadeSatiResult {
  const now = new Date();
  ...
  function findSaturnEntryDate(sign: number, startDate: Date, searchForward: boolean): Date {
    ...
    for (let i = 0; i < 30 * 365.25; i++) {
      if (getSaturnSignAtDate(date) === sign) {
...
export function calculateVarshaphal(
  birthDate: Date, birthSunLongitude: number, birthLagnaSign: number,
  targetYear: number, lat?: number, lng?: number
): VarshaphalResult {
  const hindiSigns = ["", "मेष", "वृषभ", "मिथुन", "कर्क", "सिंह", "कन्या", "तुला", "वृश्चिक", "धनु", "मकर", "कुंभ", "मीन"];
...
export function calculateShadbala(
  planets: Record<string, { sign: number; house: number; longitude: number; is_retrograde: boolean }>,
  lagnaSign: number, isDayBirth: boolean
): ShadbalaResult {
...
export function calculateAshtakavarga(
  planets: Record<string, { sign: number; longitude: number }>,
  lagnaSign: number
): AshtakavargaResult {
  const hindiSigns = ["", "मेष", "वृषभ", "मिथुन", "कर्क", "सिंह", "कन्या", "तुला", "वृश्चिक", "धनु", "मकर", "कुंभ", "मीन"];
  ...
  const totalBindu = sarvashtakavarga.reduce((a: number, b: number) => a + b, 0);

  return { binnashtakavarga, sarvashtakavarga, signStrengths };
}

export function calculateExtraDivisionalCharts(
